...
public class RechercheAmiListCellRenderer implements ObservableListRenderer<Utilisateur> {
	@Override
	public Component getListCellRendererComponent(JList<? extends Utilisateur> list, Utilisateur value, int index,
		...
		Avatar avatar = new Avatar(); // TODO : Faire en sorte qu'on puisse
										// redimensionner l'avatar
		// setSize(20, 20);
		avatar.setImage(value.getAvatar());
		String utilisateur = value.getPseudo() + "(" + value.getNom() + " " + value.getPrenom() + ")"
				+ (value == Application.getInstance().getSession().getUtilisateur() ? " (Vous)" : "");
